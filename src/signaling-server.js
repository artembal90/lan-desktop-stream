...
function createSignalingServer({ policy, pin = "", receiverPath, onEvent = () => {}, log = () => {}, graceMs = 10000, bindAddress = "0.0.0.0" }) {
  ...
  wss.on("connection", (ws, req) => {
    ...
    const reject = (code, reason, message) => { if (dead) return; dead = true; clearTimeout(joinTimer); log("WARN", "Signaling rejected", { ip, code, reason }); if (message) send(ws, message); ws.close(code, reason); const timer = setTimeout(() => ws.terminate(), 1000); timer.unref(); ws.once("close", () => clearTimeout(timer)); };
